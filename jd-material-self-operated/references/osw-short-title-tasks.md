...
The browser bridge accepts only the explicitly reviewed frontend pairs `d6069ca/index.4c6d2437.js + batch.016fe8c9.js` and `e5e2d19/index.e44ce2eb.js + batch.f9863095.js`. An unknown or ambiguous frontend still stops before imports and writes. Before adding another pair, compare the task permission, lookup, create and download exports and payloads, add failing contract fixtures, then verify native ERP/permission and exact-name lookup read-only. Never update a hash solely to suppress the version guard. A prepare-stage failure can leave the outer title audit uncertain even before a create intent exists; preserve it and reconcile the execution-stage evidence, not just an empty task search. No image regeneration or automatic task replay follows a frontend update.
