...
The Skill is a reusable starting point, not a ceiling on Codex's abilities. When blocked by a missing command, local bug or repeated waiting, read `references/autonomous-recovery.md` and investigate on the colleague's computer. Discover existing O2/native capabilities, reproduce the failure, and fix the smallest local execution-layer defect with regression tests and non-destructive application. Do not simply wait for a maintainer release. Keep protected service calls, ERP authorization, frozen scope and unknown-write evidence intact; inability to prove a write outcome is not permission to replay it. Ask the colleague only for the specific login, permission or scope decision that cannot be performed locally, not to relay an ordinary debugging task to the maintainer.

## Automatic protected-client updates

Installed protected clients check the original public GitHub stable release before normal CLI startup, at most once per six hours. No maintainer/private installation or unpacked development repository is auto-updated. Read `references/auto-update.md` for enable/disable/status controls, one-time onboarding of old installations, compatibility gates and rollback. Updates require successful release CI, pinned commit downloads and matching manifests; the release must explicitly allow the installed version without state migration. The CLI process owns an OS lock across update and the entire business run, reloads the entrypoint before business imports, and never replaces an in-flight task. Network failure keeps the usable version. Local repairs are staged for Codex's tested three-way merge, not overwritten; genuine contract/dependency/shared-runtime changes need compatibility review rather than blind upgrading. Do not turn an update issue into indefinite waiting for the maintainer: inspect the local status/candidate and follow the autonomous troubleshooting guide. Updates do not grant maintenance, change models, reset credentials or resume pauses. After an update notice, read the updated Skill before choosing new capabilities.
