...
def check_unknown_writes(batch, *, uploads_only=False):
    ...
    if ledger.exists():
        entries = manual.read_json(ledger).get('files', {}).values()
...
def export_failures(output, progress, jobs, erp):
    ...
    for segment in progress['segments'].values():
        ...
        for spu, item in report['jobs'].items():
            ...
            evidence[spu] = {'category': 'awaiting-readback' if item.get('awaitingReadback') else 'failed', 'reason': '; '.join((reason for reason in reasons if reason)) or 'incomplete material; see source receipt', 'sourcePath': str(path), 'sourceSha256': segment['reportSha256']}
    for spu, item in progress.get('heldSpus', {}).items():
        evidence.setdefault(spu, {'category': 'skipped-previous-image-failure', 'reason': item['reason'], 'sourcePath': item['path'], 'sourceSha256': item['sha256']})
