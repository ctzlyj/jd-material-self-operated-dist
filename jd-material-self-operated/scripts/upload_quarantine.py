...
def load(output):
    ...
    if record.get('formatVersion') != 1 or record.get('confirmation') != CONFIRM or record.get('allowedWrites') != [] or (record.get('replayAllowed') is not False):
    ...
    entry = expected.get(record['fileName'], {})
    if entry.get('status') not in ('uploading', 'unconfirmed', 'submitted'):
    ...
    image = batch / '全部图片' / record['fileName']
    if record['fileName'] not in names or not image.is_file() or manual.sha(image) != entry.get('sha256'):
        raise ValueError('quarantined upload has no matching frozen image')
    return {spu: {**record, 'evidence': pointer}}
...
def freeze(output, erp, token, segment, spu, filename, confirm):
    ...
    if (output / '.state/upload-quarantine.json').exists():
        existing = load(output)
        if set(existing) != {spu} or existing[spu]['batch'] != str(folder / '批次001') or existing[spu]['fileName'] != filename:
    ...
    with core._erp_execution_lock(output), core._erp_execution_lock(folder):
        ...
        if entry.get('status') not in ('uploading', 'unconfirmed', 'submitted') or not image.is_file() or manual.sha(image) != entry.get('sha256'):
